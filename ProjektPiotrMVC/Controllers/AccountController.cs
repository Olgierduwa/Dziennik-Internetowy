...
namespace ProjektPiotrMVC.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            List<AccountsViewModel> AVMList = new List<AccountsViewModel>();
            List<ApplicationUser> Konta = db.Users.ToList();
            foreach(var user in Konta)
            {
                ...
                if (Roles != null && Roles.Count > 0)
                {
                    ...
                    AVM.Rola = db.Roles.Where(m => m.Id == RoleId).FirstOrDefault();

                }
                else AVM.Rola = new IdentityRole();
                AVM.Konto = user;
                AVMList.Add(AVM);
            }
            return View(AVMList);
        ...
        public ActionResult DeleteConfirmed(string id)
        {
            ...
            // usuwanie zwiazanych z uzytkownikiem elementow
        ...
        [Authorize(Roles = "Admin")]
        public ActionResult Dziecko(string id)
        {
            return RedirectToAction("Index", "Dziecko", new { id = id } );
        }
        ...
        {
            var Roles = db.Roles.Select(m => new { RoleID = m.Id, Name = m.Name }).ToList();
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var result = await UserManager.CreateAsync(user, model.PESEL);
                if (result.Succeeded)
                {
                    IdentityManager im = new IdentityManager();
                    string RoleName = db.Roles.Where(m => m.Id == model.RoleID).Select(m => m.Name).FirstOrDefault();
                    im.AddUserToRoleByUsername(user.Email, RoleName);
                    
                    // await SignInManager.SignInAsync(user, isPersistent:false, rememberBrowser:false); // logowanie
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                return RedirectToAction("PasswordLink", "Account", new { link = callbackUrl } );
            }
        ...
        [AllowAnonymous]
        public ActionResult PasswordLink(string Link)
        {
            ViewBag.Link = Link;
            return View();
        }
